...
### Bullet shape rule (REQUIRED)

Every content slide that uses bullets must follow one of these shapes:

- **3 main bullets, each with 2 sub-bullets** (3 × 2)
- **2 main bullets, each with 3 sub-bullets** (2 × 3)

Pick whichever fits the content; do not mix freely on the same slide. For two-column comparison slides (e.g. Methods with two halves), each column independently follows 3 × 2 or 2 × 3 — and **both columns use the same shape** so the slide reads as a single comparison rather than two unrelated lists.

Sub-bullets render at level 1 (indented, smaller font, different glyph). Main bullets render at level 0.

Slides that are NOT bullet-driven (title slide, references, conclusions sandwich, callout-only slides) are exempt from this rule.
